...
def format_code(v):
    ...
    try:
        # 숫자로 들어온 종목코드를 6자리 문자로 (예: 250 -> '000250')
        ...
        with st.spinner('엑셀 데이터 정밀 분석 중...'):
            ...
            if res_f_end.status_code != 200:
                ...
                if res_top_e.status_code == 200:
                    ...
                    flow_df = pd.merge(flow_df, top_df[['종목명', '순위', '종목코드', '계좌수']], on='종목명', how='left')
                ...
                flow_df.rename(columns={'날짜': '디바신호일', '종가': '기준종가', '현재가': '현재종가'}, inplace=True)
                flow_df['상승률'] = np.where(flow_df['기준종가'] > 0, 
                                          ((flow_df['현재종가'].apply(to_num) - flow_df['기준종가'].apply(to_num)) / flow_df['기준종가'].apply(to_num) * 100).round(2), 0.0)
